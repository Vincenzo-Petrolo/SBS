...
#define P3_PRIORITY 4 //lower than hard real time tasks
...
void process3_entry()
{
    ...
    DEBUG_PRINT("Process 3 is waiting the lock\n", HEAVY_DEBUG);

    rt_sem_take(&sem_lock, 100);

    unsigned char data_to_encrypt[32 + 1];
    unsigned char data_to_decrypt[32 + 1];

    rt_memcpy(data_to_encrypt, "ciao\0", 5);
    /* encrypt */
